...
CLASS: REPORT-ONE-POPULATION — a Script Report that built its aggregates (attendance map, chart, summary) from a wider population than its rows: the Monthly Attendance Sheet took the requested company plus descendants at face value (no company fence, no native Employee row restrictions) and read every attendance row of those companies before the employee list existed, so the chart and summary counted people the caller could not see.
Changed: hrms/hr/report/monthly_attendance_sheet/monthly_attendance_sheet.py (execute: fenced_companies refuses a foreign company and descendants outside the fence never join filters.companies; the employee population — company fence + native Employee match conditions — is resolved FIRST and every attendance read is restricted to it; get_data takes the resolved population instead of querying again).
Call sites / consumers:
hrms/hr/report/monthly_attendance_sheet/monthly_attendance_sheet.py get_attendance_records same-root — Attendance.employee.isin(population).
hrms/hr/report/monthly_attendance_sheet/monthly_attendance_sheet.py get_employee_related_details same-root — company fence + Criterion.all(build_qb_match_conditions("Employee")).
hrms/hr/report/monthly_attendance_sheet/monthly_attendance_sheet.py get_attendance_summary_and_days / get_leave_summary / get_entry_exits_summary not-affected — per-employee queries keyed by the population's names and the fenced companies.
hrms/hr/report/monthly_attendance_sheet/monthly_attendance_sheet.py get_chart_data not-affected here — built from the now-fenced attendance map; its half-day counting is the next slice (REPORT-HALF-DAY-CHART).
hrms/hr/report/monthly_attendance_sheet/monthly_attendance_sheet.py get_attendance_years (whitelisted) not-affected — a list of years, no rows.
hrms/hr/report/monthly_attendance_sheet/test_monthly_attendance_sheet.py not-affected — bench suite; execute's contract (columns, data, message, chart) unchanged.
hrms/hr/report/employee_analytics/employee_analytics.py same-root — next slice in this batch (Not Set count from the authorized population).
Lock: hrms/tests/test_monthly_attendance_sheet.py (foreign company refused; a descendant outside the fence reaches neither rows nor chart; native Employee restrictions scope rows, summary and chart together; summarized view uses the same population) on hrms/tests/_qb_stub.py.
Machine-listed name collisions for the REPORT-ONE-POPULATION change:
hrms/hr/doctype/overtime_slip/overtime_slip.py:99 not-affected — OvertimeSlip.get_attendance_records is a method on another doctype; only the name collides.
hrms/hr/report/appraisal_overview/appraisal_overview.py:18 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/daily_work_summary_replies/daily_work_summary_replies.py:14 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/employee_exits/employee_exits.py:14 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/employee_leave_balance/employee_leave_balance.py:26 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/employee_leave_balance_summary/employee_leave_balance_summary.py:18 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/employees_working_on_a_holiday/employees_working_on_a_holiday.py:16 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/leave_ledger/leave_ledger.py:13 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/project_profitability/project_profitability.py:10 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/recruitment_analytics/recruitment_analytics.py:16 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/hr/report/shift_attendance/shift_attendance.py:23 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/payroll/report/accrued_earnings_report/accrued_earnings_report.py:12 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/payroll/report/employee_ctc_break_up/employee_ctc_break_up.py:353 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/payroll/report/income_tax_computation/income_tax_computation.py:32 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/payroll/report/income_tax_deductions/income_tax_deductions.py:17 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
hrms/payroll/report/professional_tax_deductions/professional_tax_deductions.py:13 not-affected — another report's module-local get_data/get_date_condition of the same name; this report's functions are not imported anywhere else.
