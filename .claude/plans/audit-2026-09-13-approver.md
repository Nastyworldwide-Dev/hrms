...
| "Still open: N02 cross-company issue recipients" | **NOT OPEN — stale ledger.** Closed by the same commit. The get_all at pwa_notifications.py:122-124 is a SORTING TIEBREAKER inside _get_ot_approver, not a broadcast; the send passes _ot_approver_can_receive:131-146 |
| "Still open: N03 OT approver policy" | **NOT OPEN AS WRITTEN — stale ledger.** Closed by d479e4c05 / 686e4aa0d, which landed AFTER that row was written. pwa_notifications.py:131-146 now calls the same _is_routed_approver + ot_row_scope.has_permission the decision uses. The real open question under that label is (c)B |
| "Script Report family hunt not yet done" | **OPEN, and far larger than implied** — 27 of 33 unfenced, 4 wrong claims in the fixed list, 4 reports actively bypass an existing hook |
...
Sight and action are decided by different code that does not agree.
  SIGHT  = the row-scope hooks in hrms/overrides/ (hooks.py:167-236 wires both permission_query_conditions and has_permission)
...
| 24 employee-owned doctypes (pay, benefits, PIP, attendance, checkins) | own + DocShare + HR (fenced). No manager visibility | HR | employee_owned_row_scope.py — **the one built correctly**: canonical identity, company fence, both hooks, integrity test |
| Appraisal | own + the TRANSITIVE reports_to chain + HR (fenced) + DocShare | same | appraisal.py:920-941 |
| Script Reports (27 of 33) | whoever holds the report role, FOR ANY COMPANY THEY TYPE INTO THE FILTER | n/a | no fence at all |
...
2. appraisal.py:884-886 _get_own_employees — raw, status-agnostic, every claimant. Its docstring at
   :930-941 ADMITS this and says identity callers should pass own_employees as the seed. kpi.py does;
...
- applicable_for gap: company_scope.py:88-90 deliberately ignores applicable_for ("it can only ever
...
CLEARED, not a negative: the replaced-approver DocShare case. share_doc_with_approver only shares when
has_permission(submit) is False, and approval_row_scope already grants the named approver submit — so
the share is never created. Harmless, but approval_row_scope.py:17-18's claim that DocShare "keeps
historical approvers working" is dead code on this fork.
...
1. **27 of 33 Script Reports unfenced.** Payroll reports take a caller-supplied `company` with NO
   validation; salary_payments_via_ecs.py:76 makes it OPTIONAL (verified), so omitting it returns every
...
2. **Four reports bypass a fence that already exists** — worse than one never written, because the
   ...
   get_all/frappe.qb (neither applies permission_query_conditions):
     ...
     attendance_day_audit.py -> utils/attendance_day_audit.py:253,274   bypasses employee_owned_row_scope
...
6. appraisal.py:884 — phantom reporting chain on the Desk list; transitive, unfenced. Fixed in kpi.py,
   not at the source.
...
1. **Four wrong entries in the "reports fenced" set.** Only SIX are fenced: Salary Register (:301),
   ...
   Summary (:20,273) and Shift Attendance (:19,293) via a SECOND helper, report_scope.py:58
   apply_employee_scope + :86 scoped_companies; Appraisal Overview (:78-109) via
   get_allowed_appraisal_employees. **Employee Leave Balance is listed as fixed and was NEVER TOUCHED**
   — employee_leave_balance.py:146-165 is a raw frappe.qb Employee query with optional, unvalidated
...
4. "Family hunt not yet done" understates the size. It reads as a tidy-up. It is 27 unfenced reports,
...
## Re-verified independently in this session
  ...
  salary_payments_via_ecs.py:76     `if filters.get("company")`     -> Tier-1 #1 CONFIRMED, optional
  employee_leave_balance.py:146-165 raw qb, optional company/department, no Active default
...
`own_employees` IS the canonical primitive — normalized, Active-only, fail-closed on ambiguity
...
Do not quote row #3's Active column.
