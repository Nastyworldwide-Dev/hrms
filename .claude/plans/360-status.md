...
| Concurrent OT decisions / RL grants | Exact OT index/locking proposal authorized by user continuation after schema question; implementation and native transaction checks remain. No production migration yet. |
| GPS freshness / accuracy / preview parity | 7a4f1161b +0773ccd72 integrated; fresh review PASS, combined frontend212/lint green and50 Python tests. Separate audit transaction durability fix active; device verification required. |
...
| OT layout / zero claim / summary races | Worker active: header order, concise copy, visible0, date/employee generation guard, preserved claim model and save gating. Existing layout mockup guides repair; no additional mockup gate under replacement agreement. |
...
| Four-month backdated OT/RL | Work date retained; calendar-month window provisional, existing code uses payroll cycles. Temporary versus permanent availability remains a policy dependency; no expiry assumed or setting enabled. Discovery must first cover the full existing filing window, instead of fixed45days. |
