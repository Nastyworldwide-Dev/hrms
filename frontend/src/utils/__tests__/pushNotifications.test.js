	...
	const window = {
		frappePushNotification: {
			serviceWorkerRegistration: {
				showNotification: (title, options) => calls.push({ title, options }),
			},
		},
	}
	load(navigator, window).showNotification(payload)
	return calls
}
...
const payload = {
	data: {
		...
		click_action: "/hrms/leave-applications/HR-LAP-1",
	},
}
...
test("Firefox: a body tap can find the URL on data, and the action button stays", () => {
	...
	assert.deepEqual(call.options.actions, [
		{ action: "/hrms/leave-applications/HR-LAP-1", title: "View Details" },
	])
})
...
test("no registration yet: nothing is shown and nothing throws", () => {
	const navigator = { userAgent: "Firefox/128.0" }
	const window = { frappePushNotification: {} }
	assert.doesNotThrow(() => load(navigator, window).showNotification(payload))
})
