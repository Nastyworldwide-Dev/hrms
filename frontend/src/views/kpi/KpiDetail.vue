<!--
  KpiDetail — ONE person's KRA detail: the hero score and ring, the cycle trend,
  the KRA bars with their targets and actuals, and the feedback count.

  Extracted so the drill-down IS the My KPI layout rather than a lookalike.
  Two implementations of this screen would drift, and the drift would be
  invisible: both would render, and only one would be right about somebody's
  performance review.

  Props:
    data     the payload from hrms.api.kpi.get_my_kpi_dashboard OR
             get_employee_kpi — the same shape by construction, pinned by
             test_the_drill_down_returns_the_same_shape_as_my_kpi
    heading  optional name shown above the hero; omitted for "my own", where
             the page title already says whose this is
-->
<template>
	<div class="flex flex-col gap-8">
					...
					<div class="border-t-2 border-divider">
						...
						>
							...
							<div class="flex flex-wrap gap-x-3 text-kra-label text-ink-500">
								<span v-if="row.target"> {{ __("Target") }} {{ formatNumber(row.target) }} </span>
		...
		<GEmptyState
			...
			:body="__('A KPI appears here once a review cycle opens')"
		/>
	</div>
</template>
...
const delta = computed(() => {
	const prev = props.data?.previous_score
