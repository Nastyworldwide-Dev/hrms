...
test("chips bucket by status word; unknown statuses only ever show under All", () => {
	...
	assert.deepEqual(
		filterTickets(list, "all").map((t) => t.name),
		["a", "b", "c", "d", "e", "f"]
	)
	assert.deepEqual(
		filterTickets(list, "open").map((t) => t.name),
		["a", "e"]
	)
	assert.deepEqual(
		filterTickets(list, "replied").map((t) => t.name),
		["b"]
	)
	assert.deepEqual(
		filterTickets(list, "resolved").map((t) => t.name),
		["c", "d"]
	)
		...
		communications: [
			{
				...
				user: { name: "agent@x.com", full_name: "Nabil Z." },
			},
			{
				name: "c1",
				...
				user: { name: "amy@x.com", full_name: "Amy E." },
			},
		],
		comments: [
			{
				...
				creation: "2026-09-01 09:30:00",
				commented_by: "agent@x.com",
				user: { full_name: "Nabil Z." },
			},
		],
	}
