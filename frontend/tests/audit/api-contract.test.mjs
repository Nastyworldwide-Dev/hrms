...
import { HRMS_PY, REPO, read, rel, resolveImport, sourceFiles, lineOf, sfc } from "./_lib.mjs"
...
function pyFiles(dir, out = []) {
	for (const entry of readdirSync(dir)) {
		...
		if (statSync(path).isDirectory()) {
			if (!["node_modules", "__pycache__", "frontend"].includes(entry)) pyFiles(path, out)
		} else if (entry.endsWith(".py") && !entry.startsWith("test_")) out.push(path)
	}
	return out
}
...
function parseSignature(paramText) {
	...
	for (const raw of splitTopLevel(paramText)) {
		...
		const hasDefault = /^\w+\s*(?::[^=]*)?=/.test(raw.replace(/\[[^\]]*\]/g, ""))
...
export function whitelistedMethods() {
	...
	const DEF_RE = /@frappe\.whitelist\([^)]*\)\s*\n(?:@[^\n]*\n)*\s*def\s+(\w+)\s*\(([^)]*(?:\([^)]*\)[^)]*)*)\)/g
	for (const file of pyFiles(HRMS_PY)) {
		const text = readFileSync(file, "utf8")
		const module = file.slice(REPO.length + 1, -3).replace(/\//g, ".").replace(/\.__init__$/, "")
		for (const m of text.matchAll(DEF_RE)) methods.set(`${module}.${m[1]}`, parseSignature(m[2]))
	}
	...
	for (const m of init.matchAll(/^from\s+(hrms\.[\w.]+)\s+import\s+\(?([^)\n]+(?:\n[^)]+)*)\)?/gm)) {
		for (const name of m[2].split(",").map((s) => s.trim().split(/\s+as\s+/).pop()).filter(Boolean)) {
			const target = `${m[1]}.${name}`
			if (methods.has(target)) methods.set(`hrms.api.${name}`, methods.get(target))
		}
	}
	return methods
}
...
const CORE = {
	login: { required: [], optional: ["usr", "pwd", "tmp_id", "otp", "cmd"], varkw: true },
	logout: { required: [], optional: [], varkw: true },
	upload_file: { required: [], optional: [], varkw: true },
	"frappe.client.get_list": {
		required: ["doctype"],
		optional: ["fields", "filters", "group_by", "order_by", "limit_start", "limit_page_length", "parent", "debug", "as_dict", "or_filters"],
		varkw: false,
	},
	"frappe.client.get": { required: ["doctype"], optional: ["name", "filters", "parent"], varkw: false },
	"frappe.client.get_value": { required: ["doctype", "fieldname"], optional: ["filters", "as_dict", "debug", "parent"], varkw: false },
	"frappe.client.set_value": { required: ["doctype", "name", "fieldname"], optional: ["value"], varkw: false },
	...
	"frappe.client.cancel": { required: ["doctype", "name"], optional: [], varkw: false },
	"frappe.client.delete": { required: ["doctype", "name"], optional: [], varkw: false },
	"frappe.client.has_permission": { required: ["doctype", "docname"], optional: ["perm_type"], varkw: false },
	"frappe.client.get_doc_permissions": { required: ["doctype", "docname"], optional: [], varkw: false },
	"frappe.desk.search.search_link": {
		required: ["doctype", "txt"],
		optional: ["query", "filters", "page_length", "searchfield", "reference_doctype", "ignore_user_permissions"],
		varkw: false,
	},
	"frappe.core.doctype.user.user.reset_password": { required: ["user"], optional: [], varkw: false },
	"frappe.core.doctype.user.user.update_password": {
		required: ["new_password"],
		optional: ["logout_all_sessions", "key", "old_password"],
		varkw: false,
	},
	"frappe.push_notification.subscribe": { required: ["fcm_token", "project_name"], optional: [], varkw: false },
	"frappe.push_notification.unsubscribe": { required: ["fcm_token", "project_name"], optional: [], varkw: false },
	"frappe.translate.get_all_translations": { required: [], optional: ["lang"], varkw: false },
	// reads frappe.form_dict itself
	"frappe.desk.reportview.get": { required: [], optional: [], varkw: true },
	"frappe.model.workflow.get_transitions": { required: ["doc"], optional: ["workflow", "raise_exception"], varkw: false },
	"frappe.model.workflow.apply_workflow": { required: ["doc", "action"], optional: [], varkw: false },
...
// ---------------------------------------------------------------------------
const RESOURCE_FACTORIES = new Set(["createResource", "createListResource", "createDocumentResource"])
...
function objectKeys(node, scope) {
	...
	for (const p of node.properties) {
		if (p.type === "SpreadElement") {
			const inner = p.argument.type === "CallExpression" && p.argument.callee.type === "Identifier" ? scope.fns.get(p.argument.callee.name) : null
			if (inner) {
...
function returnedObject(fn, scope) {
	...
	if (fn.type === "Identifier") return scope.fns.get(fn.name) || { keys: new Set(), dynamic: true }
	if (fn.type !== "ArrowFunctionExpression" && fn.type !== "FunctionExpression" && fn.type !== "ObjectMethod")
	...
	if (fn.body.type === "BlockStatement") {
		...
		if (ret?.argument?.type === "ObjectExpression") return objectKeys(ret.argument, scope)
	}
	return { keys: new Set(), dynamic: true }
}
...
function literalString(node, scope) {
	...
	if (node.type === "Identifier" && scope.consts.has(node.name)) return scope.consts.get(node.name)
	return null
}
...
export function requestSites(code, file, lineOffset = 0, absPath = null) {
	...
	for (const stmt of ast.program.body) {
		if (stmt.type === "ImportDeclaration") {
			for (const sp of stmt.specifiers) imports.set(sp.local.name, stmt.source.value)
			continue
		}
		...
		for (const d of decl.declarations) {
			...
			if (d.init?.type === "StringLiteral") scope.consts.set(d.id.name, d.init.value)
	...
	for (const stmt of ast.program.body) {
		if (stmt.type === "ExportNamedDeclaration" && stmt.declaration?.type === "VariableDeclaration")
			for (const d of stmt.declaration.declarations) if (d.id.type === "Identifier") exportedNames.add(d.id.name)
	}
	walk(ast.program, (node, parent) => {
		...
		if (name === "call" && callee.type === "Identifier" && node.arguments[0]) {
			...
			sites.push({ file, absPath, line: lineOffset + lineOf(code, node.start), url, kind: "call", ...args, doctype: null, fields: [] })
			return
		}
		if (!RESOURCE_FACTORIES.has(name) || node.arguments[0]?.type !== "ObjectExpression") return
		const opts = node.arguments[0]
		const prop = (k) => opts.properties.find((p) => p.key && !p.computed && (p.key.name === k || p.key.value === k))
		...
		const site = {
			...
			urlDynamic: Boolean(urlNode) && literalString(urlNode.value, scope) === null,
			keys: new Set(),
			dynamic: false,
			// with makeParams, submit()/fetch() arguments are INPUTS to makeParams,
			// not kwargs — unless makeParams just hands them through
			argsAreKwargs: true,
			doctype: prop("doctype") ? literalString(prop("doctype").value, scope) : null,
			fields: [],
			varName: null,
			exported: false,
		}
		...
		if (make) {
			...
			const passthrough =
				...
				fn.body.body.some((s) => s.type === "ReturnStatement" && s.argument?.type === "Identifier" && s.argument.name === fn.params[0].name)
		...
		if (fields?.value?.type === "ArrayExpression")
			site.fields = fields.value.elements.map((e) => (e?.type === "StringLiteral" ? e.value : null))
	...
	walk(ast.program, (node) => {
		...
		if (!["submit", "fetch", "reload"].includes(method) || !node.arguments.length) return
		let obj = node.callee.object
		if (obj.type === "MemberExpression" && obj.object.type === "Identifier") obj = obj.object // docList.insert.submit()
		...
		if (obj.type === "CallExpression") site = byNode.get(obj)
			...
			else if (imports.has(obj.name)) calls.push({ name: obj.name, spec: imports.get(obj.name), ...r })
		}
	...
	walk(ast.program, (node, parent) => {
		...
		if (parent?.type === "ObjectProperty" && parent.key === node && !parent.shorthand) return
		if (byVar.has(node.name)) byVar.get(node.name).dynamic = true
		else if (imports.has(node.name)) calls.push({ name: node.name, spec: imports.get(node.name), keys: new Set(), dynamic: true })
	})
	return { sites, calls }
}
...
function allSites() {
	...
	for (const { sites } of perFile) for (const s of sites) if (s.exported) exported.set(`${s.absPath}#${s.varName}`, s)
...
test("every url the PWA calls resolves to a whitelisted hrms def or an allowlisted core method", () => {
	...
	for (const s of sites) {
		...
		if (!methods.has(url) && !(url in CORE)) unknown.push(`${s.file}:${s.line} → ${url}`)
	}
	assert.deepEqual(unknown, [])
})

test("every url is a string literal the audit can read", () => {
	const dynamic = sites.filter((s) => s.urlDynamic).map((s) => `${s.file}:${s.line}`)
	...
	for (const s of sites) {
		...
		for (const k of s.keys) {
			if (!sig.varkw && !sig.required.includes(k) && !sig.optional.includes(k))
				findings.push(`${s.file}:${s.line} ${url} passes unknown kwarg \`${k}\` (accepts ${[...sig.required, ...sig.optional].join(", ") || "nothing"})`)
		}
		if (s.dynamic) continue // arguments built at call time — cannot be read statically
		for (const r of sig.required) {
			if (!s.keys.has(r)) findings.push(`${s.file}:${s.line} ${url} never passes required \`${r}\` (passes ${[...s.keys].join(", ") || "nothing"})`)
		}
	}
...
const STANDARD_FIELDS = new Set([
	"name", "owner", "creation", "modified", "modified_by", "docstatus", "idx", "parent", "parentfield", "parenttype", "_user_tags", "_comments", "_assign", "_liked_by",
])

function doctypeJson(doctype) {
	const snake = doctype.toLowerCase().replace(/\s+/g, "_")
	const candidates = [HRMS_PY]
	for (const base of [join(REPO, ".."), join(process.env.HOME || "", "verify-bench", "apps")]) {
...
const CUSTOM_FIELD_SOURCES = ["setup.py", "utils/extension_custom_fields.py", "sync/runner.py"]
...
function customFields(doctype) {
	...
	for (const source of CUSTOM_FIELD_SOURCES) {
		...
		while ((start = text.indexOf(`"${doctype}": [`, start + 1)) !== -1) {
			...
			for (const m of text.slice(start, end).matchAll(/"fieldname":\s*"(\w+)"/g)) names.push(m[1])
		}
	}
	return names
}
...
test("every field a createListResource reads exists on its doctype", () => {
	...
	const skipped = []
	for (const s of sites) {
		if (s.kind !== "createListResource" || !s.doctype || !s.fields.length) continue
		const json = doctypeJson(s.doctype)
		if (!json) {
			skipped.push(`${s.file}:${s.line} ${s.doctype} (doctype JSON not on disk)`)
			continue
		}
		const known = new Set([...STANDARD_FIELDS, ...json.fields.map((f) => f.fieldname), ...customFields(s.doctype)])
		if (json.istable) known.add("parent")
		for (const f of s.fields) {
			if (f === null || f === "*" || f.includes("`") || f.includes(" as ")) continue
			if (!known.has(f)) findings.push(`${s.file}:${s.line} ${s.doctype}.${f} is not a field of the doctype`)
		}
	}
	if (skipped.length) console.log("[api-contract] doctype JSON not found (skipped):\n  " + skipped.join("\n  "))
	assert.deepEqual(findings, [], findings.join("\n"))
})
...
test("most request sites are readable statically", () => {
	...
	console.log(
		`[api-contract] ${withUrl.length} request sites, ${static_.length} with static arguments, ${withUrl.length - static_.length} dynamic`
	)
	assert.ok(static_.length >= 60, `only ${static_.length} of ${withUrl.length} sites are statically readable`)
})
...
`
	...
	assert.ok(sig.required.includes("date") && !sites[0].keys.has("date"), "date is required and not passed")
	assert.ok(!methods.get("hrms.api.get_shifts").optional.includes("employe"), "the misspelt kwarg is unknown")
})
