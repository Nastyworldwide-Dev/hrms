...
import { FRONTEND, SRC, read, rel, resolveImport, sourceFiles, sfc, scriptText } from "./_lib.mjs"
...
for (const file of files) {
	...
	for (const m of text.matchAll(/\bimport\s*(?:([\w${},*\s]+?)\s*from\s*)?["']([^"']+)["']|\bimport\(\s*["']([^"']+)["']\s*\)/g)) {
		const spec = m[2] || m[3]
...
for (const m of read(`${SRC}/data/config/requestSummaryFields.js`).matchAll(/componentName:\s*"(\w+)"/g))
...
function namedExports(file) {
	...
	const names = []
	for (const m of text.matchAll(/^export\s+(?:const|let|function|async function|class)\s+([\w$]+)/gm)) names.push(m[1])
...
const EXPORT_EXEMPT = {
	"src/utils/geolocation.js": ["GEO_DENIED", "GEO_INSECURE", "GEO_TIMEOUT", "GEO_UNAVAILABLE", "GEO_UNSUPPORTED"], // error codes, matched by value in CheckInPanel
}
...
function testFiles() {
	...
	const walk = (dir) => {
		...
		for (const entry of readdirSync(dir)) {
			...
			} else if (/\.(test\.(js|mjs)|spec\.js|mjs)$/.test(entry) && !path.includes("/tests/audit/")) out.push(path)
		}
	}
...
test("every named export is imported by another module or reached by a test", () => {
	const dead = []
	for (const file of files) {
		if (!file.endsWith(".js") || file.includes("/src/router/") || rel(file) === "src/main.js") continue
...
const GLOBAL_PROPERTIES = new Set(["$slots", "$emit", "$attrs", "$props", "$el", "$refs", "$nextTick", "$forceUpdate", "__"])
...
test("every template identifier and component tag resolves to a binding, a global or an import", () => {
	...
	for (const file of files) {
		...
		let bindings = {}
		if (descriptor.scriptSetup || descriptor.script) bindings = compileScript(descriptor, { id: "audit" }).bindings || {}
		...
		for (const err of tpl.errors) findings.push(`${rel(file)}: template error — ${err.message || err}`)
		for (const m of tpl.code.matchAll(/_ctx\.([A-Za-z_$][\w$]*)/g)) {
			if (!GLOBAL_PROPERTIES.has(m[1])) findings.push(`${rel(file)}: template uses \`${m[1]}\` which the script never defines`)
		}
		for (const m of tpl.code.matchAll(/_resolveComponent\("([^"]+)"/g)) {
			if (!GLOBAL_COMPONENTS.has(m[1])) findings.push(`${rel(file)}: component <${m[1]}> is neither imported nor registered globally`)
		}
		for (const m of tpl.code.matchAll(/_resolveDirective\("([^"]+)"/g)) findings.push(`${rel(file)}: directive v-${m[1]} is not registered`)
	}
