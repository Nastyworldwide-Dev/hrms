...
function browser(
	...
	const context = vm.createContext({
		...
		URLSearchParams,
		document: {
			cookie: `user_id=${encodeURIComponent(user)}`,
		...
		request,
		frappeRequest: request,
		makeLoudRequest: (fetcher) => fetcher,
	...
	vm.runInContext(
		executable(source("node_modules/frappe-ui/src/resources/listResource.js")),
		context
	)
...
test("OT and notification cache stays available to its account but never another account/site", async () => {
	...
	const ot = a.requests.find(
		({ options }) => options.url === "hrms.api.get_ot_requests"
	)
	ot.resolve([
		{ name: "OT-A", employee: "account-a@example.invalid", ot_date: null },
	])
	await tick()
	const notificationFetch = a.run("notifications.list.fetch()")
	a.requests
		.find(({ options }) => options.params?.doctype === "PWA Notification")
		.resolve([
			{
				name: "NOTICE-A",
				message: "Account A private message",
				to_user: "account-a@example.invalid",
			},
		])
	...
	for (const [user, origin] of [
		...
		assert.equal(
			next.run("myOTRequests.data"),
			null,
			`OT privacy for ${user} at ${origin}`
		)
		assert.equal(
			next.run("notifications.data"),
			null,
			`notification privacy for ${user} at ${origin}`
		)
	}
})
...
test("startup removes legacy personal keys while logout removes only the departing identity", async () => {
	...
	const key = (user) =>
		JSON.stringify(["hrms:private:v1", origin, user, "hrms:my_ot_requests"])
	...
	await tick()
	assert.equal(
		store.has(JSON.stringify(["hrms:my_ot_requests"])),
		false,
		"remove legacy shared OT cache"
	)
	assert.equal(
		store.has(JSON.stringify(["nsty:remote-checkin-pending"])),
		false,
		"remove legacy remote cache"
	)
	assert.equal(
		store.has(aKey),
		true,
		"startup preserves this account's scoped cache"
	)
	...
	assert.equal(
		store.has(bKey),
		true,
		"logout preserves another identity's scoped data"
	)
	assert.equal(
		store.get(JSON.stringify(["other-app:preferences"])),
		"untouched"
	)
	...
	a.requests
		.find(({ options }) => options.url === "hrms.api.get_ot_requests")
		.resolve([
			{
				name: "OT-LATE-A",
				employee: "account-a@example.invalid",
				ot_date: null,
			},
		])
	await tick()
...
test("server realtime cache names refresh the current account's scoped resource", async () => {
	...
	assert.equal(
		b.requests.at(-1).options.params.employee,
...
test("a surviving A tab cannot fetch or cache B data after another tab changes the login", async () => {
	...
	assert.equal(
		a.requests.length,
		before,
		"stale resource must not dispatch with B's cookie"
	)
	...
	assert.equal(
		a.run("personalCacheKey('hrms:my_issues')"),
		null,
		"late-created resources cannot adopt B's identity"
	)
})
...
test("identity changes during a request prevent both resource and list completion persistence", async () => {
	...
	a.requests
		.find(({ options }) => options.url === "hrms.api.get_ot_requests")
		.resolve([
			{ name: "OT-B", employee: "account-b@example.invalid", ot_date: null },
		])
	a.requests
		.find(({ options }) => options.params?.doctype === "PWA Notification")
	...
	await tick()
	assert.equal(
		a.run("myOTRequests.data"),
		null,
		"stale fulfilled OT response must be discarded"
	)
	assert.equal(
		a.run("notifications.data"),
		null,
		"list's uncached inner fetch must also be guarded"
	)
...
test("a new login epoch discards old completions even when the final cookie user is unchanged", async () => {
	...
	await loginTab.run(
		"session.login('account-a@example.invalid', 'synthetic-password')"
	)
