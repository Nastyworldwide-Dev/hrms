...
function errorComponent(resource) {
	...
	vm.runInContext(
		executable(script("../src/components/ResourceError.vue")),
		context
	)
	return {
...
test("a list wrapper whose request failed shows the error and its loading state", () => {
	const wrapper = reactive({
		data: [],
		list: { error: new Error("403"), loading: false },
		reload() {},
	})
...
test("a healthy list wrapper and a plain resource still behave", () => {
	assert.equal(
		errorComponent({
			data: [],
			list: { error: null, loading: false },
		}).failed(),
		false
	)
	assert.equal(
		errorComponent({ error: "boom", loading: false, reload() {} }).failed(),
		true
	)
	assert.equal(
		errorComponent(undefined).failed(),
		false,
		"a missing resource never throws"
	)
})
...
test("the feed says 'all caught up' only after its own request answered empty", () => {
	...
	assert.match(
		view,
		/<GEmptyState\s+v-if="feedIsEmpty"/,
		"the empty state is gated on feedIsEmpty"
	)
	const gate =
		view.match(/const feedIsEmpty = computed\(([\s\S]*?)\)\n/)?.[1] || ""
	for (const term of [
