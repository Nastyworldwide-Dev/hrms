"""Guard: report scope's `is_hr` answers by DELEGATION to
`hrms.hr.utils.sees_all_employee_data` — one implementation of the
HR_SEE_ALL_ROLES rule. Run as `python3 hrms/tests/test_report_scope.py`."""

import ast
import unittest
from pathlib import Path

SOURCE = Path(__file__).resolve().parent.parent / "utils" / "report_scope.py"
...
class TestIsHrDelegates(unittest.TestCase):
	def setUp(self):
		...
		self.fn = next(
			node for node in ast.walk(tree) if isinstance(node, ast.FunctionDef) and node.name == "is_hr"
		)
