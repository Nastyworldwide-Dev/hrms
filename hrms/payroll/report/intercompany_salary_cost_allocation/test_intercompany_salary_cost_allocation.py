...
ALLOCATIONS = {
	...
	# EMP-002 has no allocation rows — falls back to cost tag
}

COST_TAGS = {"EMP-001": "Interco A", "EMP-002": "Interco B"}
...
class TestBuildAllocationRows(unittest.TestCase):
	...
	def test_employee_without_allocation_falls_back_to_cost_tag(self):
		rows = build_allocation_rows(SLIPS, EMPLOYER_COSTS, ALLOCATIONS, COST_TAGS)
		emp2 = [r for r in rows if r["employee"] == "EMP-002"]

		self.assertEqual(len(emp2), 1)
		self.assertEqual(emp2[0]["territory"], "Interco B")
